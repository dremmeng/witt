...
In this paper, we build on the work of La Nave and Phillips \cite{La_Nave_2019} $(W')$, who constructed a fractional Witt algebra with real parameters, and Purohit \cite{Leibniz} $(W'')$ who derived a $q$-analogue of the Leibniz rule. By combining these approaches, we extend the generators and structure functions to the complex domain, allowing for a richer class of Lie algebras with complex-valued structure constants.
We outline continuous combinatorial functions for use in $q$-deformations. We extend the Weyl-derivative to complex parameters in $s$. Then we examine a complexification of the derivative parameter on three separate Witt structures the third being a middle ground between the $q$-analogue and Philips - La Nave's work $W'''$. 
...
The Weyl derivative provides a natural extension of classical differentiation to fractional and complex orders. For a function $f(\theta) = \sum_{n=-\infty}^{\infty} a_n e^{in\theta}$ with Fourier coefficients $a_n$, the Weyl derivative of order $s \in \mathbb{C}$ is defined as:
\[
\partial^s f(\theta) = \sum_{n=-\infty}^{\infty} (in)^s a_n e^{in\theta}.
\]
This operator extends the familiar integer-order derivatives to complex parameters while maintaining key properties such as commutativity: $\partial^\nu \partial^\mu = \partial^{\nu+\mu}$ for all $\nu, \mu \in \mathbb{C}$.
...
We follow suite to analytically extend the $q$-Pochhammer symbol. We have the standard $q$-Pochhammer symbol:
\begin{align*}
...
\begin{align*}
  ...
  &= i^\mu i^\nu \sum_{n=1}^\infty (n)^\mu n^\nu a_n e^{i n \theta} + (-n)^\mu (-n)^\nu a_{-n} e^{-i n \theta} \\
  &= \sum_{n=1}^\infty (n)^{\nu+\mu} a_n e^{i n \theta} + (-n)^{\mu+\nu} a_{-n} e^{-i n \theta} \\
  &= \partial^{\nu +\mu} f \qedsymbol
\end{align*}
It is clear that $s, \nu,\mu$ can take on any value in $\C$. 
...
\end{align*}
\begin{lemma}
And in the particular case $f(z) = z^{-p}$ we have\cite{Leibniz}:
\begin{align*}
...
\subsection{La Nave and Philips' Work}
...
\begin{proof}
  We sketch the proof but it requires a direct calculation of the integral and will be subject to future work. Putting $[L^{''\alpha}_p,[L^{''\alpha}_n , L^{''\alpha}_m]]$. 
\end{proof}
\end{Conjecture}
...
Remaining to check the Jacobi identity of $W''$.
